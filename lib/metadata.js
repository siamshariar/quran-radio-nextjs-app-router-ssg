...
export async function buildMetadata({ title = "", description = "", path = "/", image = "", type = "website", playback = null } = {}) {
	...
	return {
		...
		openGraph: {
			...
			description: fullDescription,
			url,
			// quran-radio-social.png is 1200x630 — the standard large-card size.
			// Declaring width/height lets crawlers render it immediately without
			// having to fetch the image first to measure it.
			images: [{ url: resolvedImage, width: 1200, height: 630 }],
			type,
			siteName: "Quran.radio",
		},
		twitter: {
			// Must match the image's actual aspect ratio (1200x630, wide) — the
			// small square "summary" card either crops it oddly or Twitter
			// rejects it outright for not meeting the square card's expectations.
